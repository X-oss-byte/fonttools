...
class AFM(object):
    ...
    def read(self, path):
        ...
        for line in lines:
            ...
            if m is None:
                raise error(f"syntax error in AFM file: {repr(line)}")
    ...
    def parsechar(self, rest):
        m = charRE.match(rest)
        if m is None:
            raise error(f"syntax error in AFM file: {repr(rest)}")
        things = [rest[fr:to] for fr, to in m.regs[1:]]
        charname = things[2]
        del things[2]
    ...
    def write(self, path, sep="\r"):
        ...
        lines = [
            ...
            f'Comment Generated by afmLib; at {time.strftime("%m/%d/%Y %H:%M:%S", time.localtime(time.time()))}',
        ]
        ...
        # write char metrics
        lines.append(f"StartCharMetrics {repr(len(self._chars))}")
        items = [
        ...
        lines.append(f"StartKernPairs {repr(len(self._kerning))}")
        ...
        if self._composites:
            ...
            lines.append(f"StartComposites {len(self._composites)}")
            for charname, components in composites:
                line = f"CC {charname} {len(components)} ;"
                for basechar, xoffset, yoffset in components:
                    line = f"{line} PCC {basechar} {xoffset} {yoffset} ;"
                lines.append(line)
    ...
    def __getitem__(self, key):
        return self._kerning[key] if isinstance(key, tuple) else self._chars[key]
    ...
    def __repr__(self):
        if hasattr(self, "FullName"):
            return f"<AFM object for {self.FullName}>"
        else:
...
if __name__ == "__main__":
    ...
    if path := EasyDialogs.AskFileForOpen():
        afm = AFM(path)
        char = "A"
