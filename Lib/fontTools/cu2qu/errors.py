...
class ApproxNotFoundError(Error):
    def __init__(self, curve):
        message = f"no approximation found: {curve}"
...
class IncompatibleGlyphsError(Error):
    ...
    def __repr__(self):
        return f"<{type(self).__name__} {self.combined_name}>"


class IncompatibleSegmentNumberError(IncompatibleGlyphsError):
    def __str__(self):
        return f"Glyphs named {self.combined_name} have different number of segments"
...
class IncompatibleSegmentTypesError(IncompatibleGlyphsError):
    ...
    def __str__(self):
        ...
        lines = [
            f'{str(i).rjust(ndigits)}: ({", ".join(repr(t) for t in tags)})'
...
class IncompatibleFontsError(Error):
    ...
    def __str__(self):
        return f'fonts contains incompatible glyphs: {", ".join(repr(g) for g in sorted(self.glyph_errors.keys()))}'
